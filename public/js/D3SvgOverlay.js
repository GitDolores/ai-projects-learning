...
T.D3Overlay = T.Overlay.extend({
    ...
    _zoomChange: function () {
        if (!this.redraw) this.init(this.selection, this.transform);
        else this.redraw(this.selection, this.transform);
    },
    ...
    onAdd: function (map) {
        ...
        this.transform = {
            ...
            LngLatToD3Point: function (a, b) {
                ...
                var point = that.map.lngLatToLayerPoint(_lnglat);
        ...
        this.transform.pathFromGeojson = d3.geo.path().projection(
            d3.geo.transform({
        ...
        //用于确定坐标的
        if (this.redraw) this.redraw(this.selection, this.transform);
