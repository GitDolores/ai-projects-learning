...
import { ascending, getTopMenu, getHistoryMode, findRouteByPath, handleAliveRoute, formatTwoStageRoutes, formatFlatteningRoutes } from './utils'
...
router.beforeEach((to: ToRouteType, _from, next) => {
    console.debug('go to route path:', to.path)
    ...
    if (userInfo) {
        // 无权限跳转403页面
        // if (to.funcs && !isOneOfArray(to.funcs, useUserStoreHook().Funcs)) {
        //     next({ path: '/error/403' })
        // }
        ...
        if (_from?.name) {
            console.debug('from name:', _from.name)
            // name为超链接
            if (externalLink) {
            ...
            // 刷新
            console.log('whole menu is ', usePermissionStoreHook().wholeMenus)
            if (to.path !== '/login') {
                if (usePermissionStoreHook().wholeMenus.length === 0 || useUserStore().User != null) {
                    console.log('create route in route nav')
                    useUserStore()
                        .makeUserInfo()
                        .then((router: Router) => {
                            console.log('在路由前守卫中生成动态路由', router)
