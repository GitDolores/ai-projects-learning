...
// 弹窗内临时选中的图标（点击图标先临时选中，点击“确认”后才真正生效）
const tempIcon = ref('')
...
const iconItemStyle = computed((): ParameterCSSProperties => {
    return (item) => {
        if (tempIcon.value === currentActiveType.value + item) {
            return {
...
function onBeforeEnter() {
    // 打开时把当前值同步到临时选择
    tempIcon.value = inputValue.value || ''
    if (!tempIcon.value) return
    currentActiveType.value = tempIcon.value.substring(0, tempIcon.value.indexOf(':') + 1)
    const name = tempIcon.value.substring(tempIcon.value.indexOf(':') + 1)
    // 寻找当前图标在第几页
    const curIconIndex = copyIconList[currentActiveType.value].findIndex((i) => i === name)
...
function onChangeIcon(item) {
    tempIcon.value = currentActiveType.value + item
}
...
function onConfirm() {
    inputValue.value = tempIcon.value
