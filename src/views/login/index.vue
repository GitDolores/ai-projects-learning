...
import { DtoCaptchaRes, getVCodeInfo } from '@/api/user'
...
const vCodeInfo = ref({} as DtoCaptchaRes)

getVCodeInfo().then((resp) => {
    vCodeInfo.value = resp
})
                ...
                .then((res) => {
                    if (res.Status === 'Y') {
                        useUserStore()
                            ...
                            .then(() => {
                                // 获取后端路由
                                console.log('create route in login')
...
onMounted(() => {
    const oldValue = sessionStorage.getItem('EnterLogin')
    if (oldValue !== 'true') {
        sessionStorage.setItem('EnterLogin', 'true')
        location.reload()
    } else {
        sessionStorage.removeItem('EnterLogin')
    }
})
</script>
            ...
            <div class="login-box">
                <div class="login-form">
                    ...
                    <el-form v-if="currentPage === 0" ref="ruleFormRef" :model="ruleForm" :rules="loginRules" size="large">
                        ...
                        <Motion v-show="vCodeInfo.Enable === 'Y'">
                            <el-form-item prop="VerifyCode">
                                <el-input v-model="ruleForm.captcha" clearable :placeholder="t('login.pureVerifyCode')" :prefix-icon="useRenderIcon(Keyhole)">
                                    <template v-slot:append>
                                        <img :src="'data:gif;base64,' + vCodeInfo.GifB64" @click="refreshVCode" />
                                    </template>
                                </el-input>
                            </el-form-item>
                        </Motion>
                        ...
                        <Motion :delay="300">
                            <el-form-item>
                                <div class="w-full h-[20px] flex justify-between items-center">
                                    <el-button v-for="(item, index) in operates" :key="index" class="w-full mt-4!" size="default" @click="useUserStoreHook().SET_CURRENTPAGE(index + 1)">
                                        {{ t(item.title) }}
                                    </el-button>
                                </div>
                            </el-form-item>
