...
export const getPickerShortcuts = (): Array<{
    ...
    return [
        ...
        },
        {
            ...
            value: () => {
                const today = new Date()
                const startOfLastWeek = new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay() - 7 + (today.getDay() === 0 ? -6 : 1))
                startOfLastWeek.setHours(0, 0, 0, 0)
                const endOfLastWeek = new Date(startOfLastWeek.getTime() + 6 * 24 * 60 * 60 * 1000 + 23 * 60 * 60 * 1000 + 59 * 60 * 1000 + 59 * 1000 + 999)
